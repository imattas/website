...
- [bk_n_forth.apk](https://github.com/BYU-CSA/BYUCTF-2026/blob/main/rev/back_and_forth/bk_n_forth.apk)
```
...
- [Repository folder](https://github.com/BYU-CSA/BYUCTF-2026/tree/main/rev/back_and_forth)
- [rev/back_and_forth/README.md](https://github.com/BYU-CSA/BYUCTF-2026/blob/main/rev/back_and_forth/README.md)
- [rev/back_and_forth/bk_n_forth.apk](https://github.com/BYU-CSA/BYUCTF-2026/blob/main/rev/back_and_forth/bk_n_forth.apk)
- [rev/back_and_forth/brute.py](https://github.com/BYU-CSA/BYUCTF-2026/blob/main/rev/back_and_forth/brute.py)
- [rev/back_and_forth/gen_bytes.cpp](https://github.com/BYU-CSA/BYUCTF-2026/blob/main/rev/back_and_forth/gen_bytes.cpp)
- [rev/back_and_forth/Main.java](https://github.com/BYU-CSA/BYUCTF-2026/blob/main/rev/back_and_forth/Main.java)

---

## Recon / Initial Analysis

This page imports the BYUCTF 2026 source material for the Reverse Engineering challenge **Back and Forth**. The prompt is kept separate from the solve notes, and upstream artifacts are linked so the challenge can be replayed from the original repository.
...
Source: [rev/back_and_forth/README.md](https://github.com/BYU-CSA/BYUCTF-2026/blob/main/rev/back_and_forth/README.md)
...
![App Running](https://raw.githubusercontent.com/BYU-CSA/BYUCTF-2026/main/rev/back_and_forth/image.png)
...
```java
 private void checkFlag(String flag, final Consumer<Boolean> callback) {
    ...
    for (int i = 0; i < this.underscores.length; i++) {
    ...
    int i2 = R.string.s;
    if (!flag.startsWith(getString(i2))) {
        callback.accept(false);
        return;
    }
    if (!check1(flag)) {
...
To solve this part, I recreated the logic in my own script(s), included with this as [gen_bytes.cpp](https://github.com/BYU-CSA/BYUCTF-2026/blob/main/rev/back_and_forth/gen_bytes.cpp) and [Main.java](https://github.com/BYU-CSA/BYUCTF-2026/blob/main/rev/back_and_forth/Main.java). I ran the generation in C++, then did the generation in Java, then actually processed and printed that back in C++. I did run into some trouble with integer conversions between the two so this took longer than I thought it would, but the part of the flag is `styl3s`, meaning we have: `byuctf{*******_*******_4ndr01d_****_styl3s_28f6}`.
...
```java
        ListenableFuture<byte[]> wasmLoadFuture = Futures.submit(new Callable<byte[]>() {
            @Override
...
Notice that it's populating `edx` / arg3? It's only taking the hash of 4 bytes, which MEANS that we can brute force just those four bytes and finally have our full flag!! I threw together [brute.py](https://github.com/BYU-CSA/BYUCTF-2026/blob/main/rev/back_and_forth/brute.py) pretty quickly and ta-da! We have our last chunk: `c0d3`.
