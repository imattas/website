...
- Diffie-Hellman security relies entirely on private keys staying secret — leaking either `a` or `b` collapses the whole exchange.
- Given a leaked private key and the other party's public key, the shared secret is a single modular exponentiation: `s = B^a mod p` (or `s = A^b mod p`).
- The shared secret is rarely used directly; common derivations are SHA-256 of its string or byte form, truncated to AES key sizes.
- When the cipher/key-derivation isn't specified, brute-force the combinations (AES-CBC/ECB/CTR/XOR × multiple key derivations) and grep for the flag format.
- pycryptodome provides `pow`, `long_to_bytes`, and the AES modes needed for the full solve.
